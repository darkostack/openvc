# Openvc Coding Conventions and Style
...
- C
  - Openvc uses and enforces the ISO9899:1999 (aka ISO C99, C99) C language standard as the minimum.
- C++
  - Openvc uses and enforces the ISO14882:2003 (aka ISO C++03, C++03) C++ language standard as the minimum.
- Extensions
...
- Openvc uses the `make pretty` build target to reformat code and enforce code format and style.  The `make pretty-check` build target is included in Openvc's continuous integration and must pass before a pull request is merged.
...
- Case
  ...
  - All Openvc class, namespace, structure, method, function, enumeration, and type names in the C/C++ language shall be in *upper camel case*.  Exception: the top level Openvc namespace 'ot'.
  - All Openvc instantiated names of instances of classes, namespaces, structures, methods, functions, enumerations, and types as well as method and function parameters in the C++ language shall be in *lower camel case*.
- Symbol Qualification
  - All Openvc C public data types and free functions should have `ot` prepended to their name.
  - All Openvc C++ code should be in the ‘ot’ top-level namespace.
- Scope
...
- Openvc uses and enforces Python 3.
